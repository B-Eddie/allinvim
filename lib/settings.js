...
const defaultOptions = {
  ...
  // These styles are injected on every page load, and take precedence over Vimium's own
  // stylesheet, so they reference the theme tokens (see lib/theme.js) rather than fixed colors.
  // Otherwise the hint markers would keep the Trigger palette while the rest of the UI followed
  // the user's chosen theme.
  userDefinedLinkHintCss: `\
div > .vimiumHintMarker {
/* linkhint boxes: flat chip on the theme's inset surface */
background: var(--surface-inset);
border: 1px solid var(--color-signal-lime);
border-radius: 4px;
}
...
`,
  ...
  // The color theme, applied to every Everything Vim surface (options, popup, Vomnibar, HUD, help
  // dialog, command listing, in-page hints, the mode indicator). The available names, and the
  // palette each one maps to, live in lib/theme.js.
  theme: "trigger-dark",
  // Everything Vim edit-mode indicator. position: "corner" (bottom-right chip),
  // "field" (anchored to the active textbox), or "hidden". docsStyle: "bar"
  // (full-width bottom bar, Vim-style) or "chip" (small bottom-right card).
  everythingVimIndicatorPosition: "corner",
  everythingVimDocsIndicator: "bar",
  // Relative line numbers in the margin when editing Google Docs.
...
const Settings = {
  ...
  migratePreEverythingVim(settings) {
    // The "GeneralVim" settings keys were renamed to everythingVim* for the "Everything Vim"
    // rebrand. Carry any stored values across to the new keys, then drop the old ones, so
    // existing users keep their indicator preferences.
    const renames = {
      generalVimIndicatorPosition: "everythingVimIndicatorPosition",
      generalVimDocsIndicator: "everythingVimDocsIndicator",
    };
  ...
  migrateSettingsIfNecessary(settings) {
    ...
    settings = this.migratePreEverythingVim(settings);
    return settings;
  },
