...
const THEME_CACHE_KEY = "generalVimTheme";
...
function applyTheme(name, { cache = true } = {}) {
  ...
  try {
    ...
    if (!root) return;
    if (appliedThemeName == theme.name && root.dataset.generalvimTheme == theme.name) return;
    for (const [property, value] of Object.entries(theme.properties)) {
      root.style.setProperty(property, value);
    }
    root.dataset.generalvimTheme = theme.name;
...
function watchTheme() {
  ...
  const api = globalThis.GeneralVimTheme;
...
if (globalThis.GeneralVimTheme == null) {
  globalThis.GeneralVimTheme = GeneralVimTheme;
