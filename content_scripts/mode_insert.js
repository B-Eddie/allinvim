class InsertMode extends Mode {
  constructor(options) {
    ...
    const handleKeyEvent = (event) => {
      ...
      // We always enter insert mode when the page initially selects a text area. That means this
      // mode should suppress keys and handle typing, even if the focus is a brand-new selection
      // (so isActive is technically false until the focus event propagates). Check for a
      // pass-next-key key.
  ...
  isActive(event) {
    ...
    // If there is a focusable element, the user is trying to use it. We should be in insert mode.
