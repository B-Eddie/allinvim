...
const LocalHints = {
  ...
  getLocalHintsForElement(element) {
    ...
    // Perf: uses _hasAngularCached set once per getLocalHints() call instead
    // of getElementsByClassName per activation. Skipped entirely when no
    // ng-scope elements exist (common case).
    if (!this.checkForAngularJs || this._angularCacheValid !== this._hasAngularCached) {
      this._angularCacheValid = this._hasAngularCached;
      if (!this._hasAngularCached) {
        this.checkForAngularJs = () => false;
      } else {
    ...
    if (!isClickable && this._hasAngularCached) isClickable = this.checkForAngularJs(element);
  ...
  getLocalHints(requireHref) {
    ...
    // Perf: hoist AngularJS detection out of per-element loop. Previously
    // getLocalHintsForElement lazily created a closure per call that did
    // getElementsByClassName("ng-scope") on first element. Do it once here.
    const hasAngular = document.getElementsByClassName("ng-scope").length > 0;
    this._hasAngularCached = hasAngular;

    // Find all elements, recursing into shadow DOM if present.
    // NOTE: must use "*" (matching upstream philc/vimium) so that shadow hosts
    // which are themselves unhintable -- e.g. custom elements like
    // <d2l-my-courses-enrollment-card> -- are still pierced. A targeted
    // HINTABLE_SELECTOR here silently drops entire shadow trees. Keep DOM order
    // for overlap detection (see below).
    const getAllElements = (root, elements) => {
      if (elements == null) elements = [];
      // Avoid Array.from() intermediate alloc; iterate live NodeList directly.
      const nodeList = root.querySelectorAll("*");
      for (let i = 0; i < nodeList.length; i++) {
        const element = nodeList[i];
        // Perf: cheap detached/display:none reject BEFORE expensive
        // getVisibleClientRect (which forces layout + getComputedStyle).
        // getClientRects() is cheap and filters display:none/detached.
        try {
          if (!element.getClientRects || element.getClientRects().length === 0) {
            // Still need shadow-root recursion: a hidden host can own
            // visible shadow children (rare). Recurse but skip hint check.
