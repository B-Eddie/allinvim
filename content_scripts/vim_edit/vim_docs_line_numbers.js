(function () {
  ...
  function scheduleLineUpdate() {
    ...
    if (now - lastRun < UPDATE_MIN_INTERVAL) {
      ...
      pendingTimer = setTimeout(function () {
        pendingTimer = 0;
        lastRun = (typeof performance !== "undefined" && performance.now)
          ? performance.now()
          : Date.now();
        try {
          updateLineMarkers();
  ...
  function updateLineMarkers() {
    ...
    try {
      ...
      if (
        Math.abs(caretTopDoc - lastCaretTop) < 2 && Math.abs(caretRect.left - lastCaretLeft) < 2 &&
      ...
      if (tiles.length) {
        const minLeft = tiles.reduce(
          (min, el) => Math.min(min, el.getBoundingClientRect().left + window.scrollX),
          Infinity,
        );
  ...
  function addEventListeners() {
    ...
    const possibleScrollContainers = document.querySelectorAll(
      ".kix-appview-editor, .docs-scrollable",
    );
  ...
  function checkInitialState() {
    try {
      api.storage.sync.get(["enabled", "lineNumbersEnabled"], (data) => {
        try {
          globalEnabled = (typeof data.enabled !== "undefined") ? !!data.enabled : true;
        } catch (_) {
          globalEnabled = true;
        }
        try {
          lineNumbersPref = (typeof data.lineNumbersEnabled !== "undefined")
            ? !!data.lineNumbersEnabled
            : true;
        } catch (_) {
