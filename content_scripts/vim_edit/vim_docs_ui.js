(function () {
  ...
  class VimUIV2Internal {
    ...
    ensureIndicator() {
      ...
      this.ind.innerHTML = '<div class="vim-bar-left"></div><div class="vim-bar-right"></div>';
      // Keep caret observer regardless of theme so block cursor works
      try {
        document.body.appendChild(this.ind);
      } catch (_) {
        try {
          document.documentElement.appendChild(this.ind);
        } catch (_) {}
      }
      try {
        this._cacheBarEls();
    ...
    setCmdline(type, text, pos) {
      ...
      this.cmdline = {
        type: type,
        text: text || "",
        pos: typeof pos === "number" ? pos : (text || "").length,
      };
    ...
    setMessage(text, isError) {
      ...
      self._msgTimer = setTimeout(function () {
        try {
          self.clearMessage();
        } catch (_) {}
      }, ttl);
    ...
    _modeLabel() {
      ...
      switch (this.modeText) {
        case "insert":
          return this._replaceMode ? "-- REPLACE --" : "-- INSERT --";
        case "visual":
          return "-- VISUAL --";
        case "visualLine":
          return "-- VISUAL LINE --";
        case "visualBlock":
          return "-- VISUAL BLOCK --";
        default:
          return ""; // Vim shows nothing in NORMAL
      }
    }
    ...
    render() {
      ...
      if (this.theme !== "vim") {
        ...
        var disp = (this.modeText === "visualLine")
          ? "VISUAL LINE"
          : (this.modeText || "").toUpperCase();
        ...
        if (this.modeText === "normal") {
          this.ind.style.backgroundColor = "#F0F4F9";
          this.ind.style.color = "#1f1f1f";
        } else if (this.modeText === "insert") {
          this.ind.style.backgroundColor = "#D9EAD3";
          this.ind.style.color = "#274e13";
        } else {
          this.ind.style.backgroundColor = "#D9D2E9";
          this.ind.style.color = "#351c75";
        }
        if (this.cmdline) {
          this.ind.style.backgroundColor = "#1d2021";
          this.ind.style.color = "#ebdbb2";
        }
        if (this.message && this.message.isError) {
          this.ind.style.backgroundColor = "#cc241d";
          this.ind.style.color = "#fbf1c7";
        }
        return;
      }
    ...
    _scheduleCursorRetry() {
      ...
      var tick = function () {
        ...
        try {
          h = caret
            ? parseFloat((caret.style.height || "").slice(0, -2)) ||
              parseFloat(getComputedStyle(caret).height) || caret.getBoundingClientRect().height
            : 0;
        } catch (_) {}
    ...
    _ensureCaretObserver() {
      ...
      try {
          ...
          for (var i = 0; i < mutations.length; i++) {
            ...
            if (m.type === "childList") {
              for (var a = 0; a < m.addedNodes.length; a++) {
                ...
                if (
                  n.nodeType === 1 &&
                  (n.matches && (n.matches(".kix-cursor-caret") || n.matches(".kix-cursor")) ||
                    (n.querySelector && n.querySelector(".kix-cursor-caret, .kix-cursor")))
                ) {
                  touched = true;
                  break;
                }
              ...
              for (var r = 0; r < m.removedNodes.length; r++) {
                ...
                if (
                  nn.nodeType === 1 &&
                  (nn.matches && (nn.matches(".kix-cursor-caret") || nn.matches(".kix-cursor")) ||
                    (nn.querySelector && nn.querySelector(".kix-cursor-caret, .kix-cursor")))
                ) {
                  touched = true;
                  break;
                }
              ...
              m.type === "attributes" && m.target && m.target.matches &&
              m.target.matches(".kix-cursor-caret, .kix-cursor")
            ) {
              touched = true;
              break;
            }
        ...
        this._caretObserver.observe(document.documentElement, {
          childList: true,
          subtree: true,
          attributes: true,
