(() => {
  ...
  function isDigitToken(t) { return typeof t === 'string' && t.length === 1 && /[0-9]/.test(t); }
  function isSingleCharToken(t) { return typeof t === 'string' && t.length === 1; }
  ...
  class VimMotionParser {
    ...
    isBinding(token) {
      if (typeof token === 'string' && token.length === 1 && token >= '0' && token <= '9') return true;
    ...
    isCommandBinding(token) {
      ...
      return !!(root && (root.children.has(token) || (root.children.has(PLACEHOLDER_CHAR) && isSingleCharToken(token))));
    }
