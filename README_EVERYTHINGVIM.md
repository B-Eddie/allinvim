...
1. **Overlay editor open?** It owns the keyboard outright. `Esc` commits,
   `Ctrl-C` discards. Nothing reaches the page or Everything Vim.
2. **Everything Vim overlay visible?** (link hints, Vomnibar, help, HUD) → yield, let
   Everything Vim win — except case 1.
3. **Google Docs?** (memoized `isGoogleDocs()`) → Docs executor. Boots
   **NORMAL** always; `Esc` leaves insert, insert ops tracked for `.` repeat.
   ...
   `data-everythingvim-mode` attribute + MutationObserver (Docs recreates caret
   nodes constantly).
4. **Plain editor focused?** (`textarea` / text-like `input` /
   ...
   `stopImmediatePropagation()` so Everything Vim's `j/k/f` never fires. Second `Esc`
...
both layers. The edit router enforces the same rule as Everything Vim navigation —
...
Pick one under **Options → Everything Vim → Theme** (the `theme` setting). The
...
no Everything Vim HUD popup. Labels never use `--` decoration (`NORMAL`, `INSERT`,
...
- `everythingVimIndicatorPosition`: `corner` (bottom-right chip, default) |
  `field` (small label pinned to the active textbox) | `hidden`.
- `everythingVimDocsIndicator`: `bar` (full-width Vim-style bottom bar with mode
...
`window.__EVERYTHINGVIM_HANDLE_DOCS_KEY__` and keep no state of their own; there is
