...
// Reflects the enabled state in the toolbar icon (enabled / partial / disabled).
function updateActionIcon(tabId, enabledState) {
  let whichIcon;
  ...
  chrome.action.setIcon({ path: iconSet[whichIcon], tabId });
}

// Last pushed enabled-state per tab, bounding refreshEnabledState
// broadcasts to one per actual state transition.
const tabRefreshStateCache = {}; // tabId -> "disabled"|"partial"|"enabled"
...
const sendRequestHandlers = {
  ...
  async initializeFrame(_request, sender) {
    ...
    // Reload Settings first: a rules save immediately followed by a reload
    // can otherwise be answered from a copy that hasn't caught up yet.
    try {
      await Settings.load();
    } catch (_) {}
    ...
    const isTopFrame = sender.frameId == 0;
    if (isTopFrame) {
      updateActionIcon(sender.tab.id, enabledState);
    }
  ...
  // Re-evaluates exclusion rules on demand and pushes the result to every
  // frame of the tab. Content scripts otherwise only learn the enabled state
  // on navigation, so a freshly saved exclusion rule would not take effect
  // until reload. Returns the fresh state to the calling frame.
  // Broadcasts are bounded: one per actual state transition per tab.
  //
  // Freshness: the caller passes the just-saved rules with the message, so
  // this never serves a stale answer from our own Settings copy (which
  // reloads asynchronously and can lose the race with this very message).
  // Without caller rules (boot-time refresh), force a storage reload first.
  async refreshEnabledState(request, sender) {
    let enabledState;
    if (request && Array.isArray(request.rules)) {
      enabledState = exclusions.isEnabledForUrl(sender.tab.url, request.rules);
    } else {
      try {
        await Settings.load();
      } catch (_) {}
      enabledState = exclusions.isEnabledForUrl(sender.tab.url);
    }
    try {
      updateActionIcon(sender.tab.id, enabledState);
    } catch (_) {}
    try {
      const key = !enabledState.isEnabledForUrl
        ? "disabled"
        : (enabledState.passKeys.length > 0 ? "partial" : "enabled");
      if (tabRefreshStateCache[sender.tab.id] !== key) {
        tabRefreshStateCache[sender.tab.id] = key;
...
chrome.tabs.onRemoved.addListener(function (tabId) {
  ...
  delete tabRefreshStateCache[tabId];
