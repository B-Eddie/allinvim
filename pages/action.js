...
const ActionPage = {
  ...
  async isVimiumInstalledInTab(tabId) {
    try {
      ...
      //
      // This is bounded: a content script listener which returns true without ever calling
      // sendResponse keeps the channel open forever, so the promise would never settle and the rest
      // of init() (attaching every click listener) would never run, leaving the popup inert. A
      // missing content script rejects immediately instead, so a timeout still means "installed".
      await Promise.race([
        chrome.tabs.sendMessage(tabId, { handler: "isVimiumInstalledInTab" }),
