...
export async function init() {
  ...
  for (const el of document.querySelectorAll("input, textarea, select")) {
    ...
    // Selects may only fire "change" in some browsers; cover both.
...
// Fill the theme dropdown from lib/theme.js, so the available themes and their labels have exactly
// one definition and the dropdown can never drift from the palette that is actually applied.
...
function setFormFromSettings(settings) {
  ...
  // A settings backup, or an older build, may name a theme this build no longer ships. In that
  // case the select has no matching option and reports no value, which would leave the dropdown
  // blank and save an empty theme. Show the default instead.
  const themeSelect = getOptionEl("theme");
  if (!themeSelect.value) {
    themeSelect.value = Settings.defaultOptions.theme;
  }
